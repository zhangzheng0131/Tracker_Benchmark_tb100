...
[img_files, pos, target_sz, resize_image, ground_truth, ...
    video_path, resize_scale] = load_video_info(video_path, clips{videoInd});
...
for frame = 1:length(ground_truth)-4
    ...
    im = imread([video_path '/imgs/' img_files(frame).name]);
    imcolor = im;
    if size(im,3) > 1,
        im = rgb2gray(im);
    end
