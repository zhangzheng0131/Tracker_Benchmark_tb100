...
% errCenterAll=[];
% errCoverageAll=[];
%by zhangzheng 2017/3/23
%seq_length = results.len; for CFWLB its res has less results than standard
seq_length=results.len;
if size(results.res,1)<size(rect_anno,1)
    seq_length=size(results.res,1);
    rect_anno=rect_anno(1:seq_length,:);
end
%by zhangzheng
